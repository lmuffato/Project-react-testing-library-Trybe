...
test('Testa se aparece No favorite pokemon found, se não existir favoritados', () => {
  const { getByText } = renderWithRouter(<FavoritePokemons pokemons={ [] } />);
  const notFoundText = getByText('No favorite pokemon found');
  expect(notFoundText).toBeInTheDocument();
});

test('Teste se são exibidos cards dos pokemons favoritados', () => {
  const favPokes = [
    {
      id: 25,
      name: 'Pikachu',
...
test('O card não é exibido se ele não estiver favoritado', () => {
  const favPoke = [
    {
      id: 25,
      name: 'Pikachu',
