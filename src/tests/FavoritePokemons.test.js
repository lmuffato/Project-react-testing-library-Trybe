...
describe('Testa o componente "FavoritePokemons"', () => {
  ...
  it('Teste se é exibido todos os cards de pokémons favoritados.', () => {
    ...
    const onlySelectedPokemons = getAllByText('More details');

    expect(onlySelectedPokemons.length).toBe(pokemons.length);
  });
  ...
  it('Teste se nenhum card de pokémon é exibido, se ele não estiver favoritado.', () => {
    ...
    const onlySelectedPokemons = queryByText('More details');

    expect(onlySelectedPokemons).not.toBeInTheDocument();
  });
});
